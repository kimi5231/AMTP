...
public:
	...
	bool Add(int x)
	{
		...
		while (true) {
			...
			if ((findlevel != -1) && (target->removed == false)) {
				while (target->fully_linked == false) {} // Wait until the node is fully linked
				return false; // Element already exists
			}

			bool valid = true;
			int highest_level = 0;
			int num_nexts = (findlevel != -1) ? target->num_nexts : highest_level;
			for (int i = 0; i < num_nexts; ++i) {
				pred[i]->lock();
				if ((false == pred[i]->removed) && (pred[i]->next[i] == target)) {
					highest_level = i;
					continue; // Valid predecessor at this level
				}
				else {
					valid = false;
					break; // Invalid predecessor, need to retry
				}
			}

			if (false == valid) {
				for (int i = 0; i <= highest_level; ++i)
					pred[i]->unlock(); // Unlock previously locked predecessors
				continue;
			}

			int new_num_nexts = 1;
			while (new_num_nexts < MAX_NEXTS && rand() % 2 == 0) {
				new_num_nexts++;
			}
			SK_NODE* new_node = new SK_NODE(x, new_num_nexts);
			for (int i = 0; i < new_num_nexts; ++i) {
				new_node->next[i] = curr[i];
			}
			for (int i = 0; i < new_num_nexts; ++i) {
				pred[i]->next[i] = new_node;
			...
			for (int i = 0; i < highest_level; ++i) {
				pred[i]->unlock(); // Unlock all predecessors after linking the new node
			}
			new_node->fully_linked = true; // Mark the new node as fully linked
			return true; // Element added successfully
		}
	}
	bool Remove(int x)
	{
		...
		while (true) {
			...
			for (int i = 0; i < victim->num_nexts; ++i) {
				pred[i]->lock();
				if ((false == pred[i]->removed) && (pred[i]->next[i] == victim)) {
					highest_level = i;
					continue; // Valid predecessor at this level
				}
				else {
			...
			for (int i = victim->num_nexts - 1; i >= 0; --i) {
				pred[i]->next[i] = victim->next[i]; // Bypass the victim node
			}
			for (int i = 0; i < victim->num_nexts; ++i) {
				pred[i]->unlock(); // Unlock all predecessors after bypassing the victim node
			}
			victim->unlock();
			return true;
		}
